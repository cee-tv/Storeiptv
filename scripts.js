...
function setupChannelList() {
  ...
  countDisplay.textContent = `Total${totalCount !== 1 ? '' : ''}: ${totalCount}`;
}
...
// Sidebar toggle and theme switcher logicconst sidebar = document.getElementById('sidebar');
...
// Prevent clicks outside sidebar from affecting it
document.addEventListener('click', (event) => {
  if (!sidebar.contains(event.target)) {
    // Sidebar click action logic (if needed)
    // For example, you might close or hide the sidebar here
    sidebar.style.display = 'none'; // Adjust this as per your sidebar toggle method
  }
});

// Allow clicks inside the sidebar to propagate
sidebar.addEventListener('click', (event) => {
  event.stopPropagation();  // Prevent clicks inside sidebar from bubbling up to document
});
